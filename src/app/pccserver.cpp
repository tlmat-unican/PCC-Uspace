...
int main(int argc, char* argv[]) {
  ...
  while (true) {
    if (UDT::INVALID_SOCK ==
        (udt_socket = UDT::accept(serv, (sockaddr*)&clientaddr, &addrlen))) {
...
#endif
{
  ...
  while (true) {
    ...
    while (ssize < size) {
      if (UDT::ERROR ==
          (ss = UDT::send(sender, data + ssize, size - ssize, 0))) {
...
#endif
{
  ...
  while (true) {
    ...
    cout << perf.mbpsSendRate << "\t\t" << perf.msRTT << "\t"
